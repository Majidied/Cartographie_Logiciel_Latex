...
\begin{frame}{Définition Simple}
  ...
  \begin{itemize}
    \item Comment découper l'application en morceaux
    \item Comment ces morceaux communiquent entre eux
    \item Quelles règles ils doivent respecter
  \end{itemize}
...
\begin{frame}{Pourquoi C'est Important~?}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Une bonne architecture permet~:}
        \begin{itemize}
          \item [\checkmark] \textbf{Comprendre} le code facilement
          \item [\checkmark] \textbf{Modifier} sans tout casser
          \item [\checkmark] \textbf{Réutiliser} des morceaux
          \item [\checkmark] \textbf{Travailler en équipe}
          \item [\checkmark] \textbf{Éviter les bugs}
          \item [\checkmark] \textbf{Améliorer les performances}
        \end{itemize}
      \end{block}
    \end{column}
    
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Une mauvaise architecture}
        = un \textbf{code spaghetti}
        
        \vspace{0.3cm}
        
        \begin{itemize}
          \item [\texttimes] Tout est mélangé
          \item [\texttimes] Impossible de s'y retrouver
          \item [\texttimes] Ajouter une fonctionnalité = tout casse~!
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
