...
\begin{frame}{Architecture Monolithique Modulaire}
  ...
  \textbf{Quand l'utiliser~?} Application moyenne à grande, équipe moyenne, migration vers microservices
...
\begin{frame}{Architecture Client-Serveur}
  ...
  \begin{block}{Analogie}
    Un restaurant~:
    \begin{itemize}
      \item Le client = vous, à votre table
      \item Le serveur (de restaurant) = prend vos commandes et les apporte
      \item La cuisine = prépare les plats
    \end{itemize}
  \end{block}
...
% =====================================================
\begin{frame}{Architecture en Couches (Layered)}
  
  \textbf{C'est quoi~?} Organisation \textbf{logique} du code en couches empilées.
  
  \vspace{0.3cm}
  
  \begin{alertblock}{Différence avec N-Tiers}
    \begin{itemize}
      \item \textbf{N-Tiers} = physique (machines différentes)
      \item \textbf{En Couches} = logique (dans le code)
    \end{itemize}
  \end{alertblock}
  
  \vspace{0.3cm}
  
  \textbf{Couches typiques~:}
  \begin{enumerate}
    \item Couche Présentation (Controllers, UI)
    \item Couche Application (Use Cases, Workflows)
    \item Couche Métier / Domain (Logique métier, Entities)
    \item Couche Services (Services externes, APIs)
    \item Couche Persistance (Repository, DAO)
  \end{enumerate}
  
  \vspace{0.3cm}
  
  \textbf{Quand l'utiliser~?} La plupart des applications, combiné avec d'autres patterns
...
\begin{frame}{Architecture Microservices (1/2)}
  ...
  \begin{block}{Analogie}
    ...
    \begin{itemize}
      \item La boulangerie vend du pain
      \item La pharmacie vend des médicaments
      \item La banque gère l'argent
    \end{itemize}
...
% =====================================================
\begin{frame}{Architecture Hexagonale (Ports \& Adapters)}
  
  \textbf{C'est quoi~?} Mettre la \textbf{logique métier au centre}, et l'isoler de tout le reste (BD, interfaces, APIs externes).
  
  \vspace{0.3cm}
  
  \textbf{Le principe~:}
  \begin{itemize}
    \item Au \textbf{centre}~: le domaine métier (la vraie valeur de l'application)
    \item Autour~: des \textbf{ports} (interfaces)
    \item Encore autour~: des \textbf{adaptateurs} (implémentations concrètes)
  \end{itemize}
  ...
  \begin{block}{Analogie}
    ...
    \begin{itemize}
      \item Le \textbf{port} = la prise dans le mur (interface standardisée)
      \item Les \textbf{adaptateurs} = les différents chargeurs qu'on peut brancher
      \item Le \textbf{centre} (métier) = l'appareil qu'on veut charger
    \end{itemize}
  \end{block}
  
  \vspace{0.3cm}
  
  \textbf{Quand l'utiliser~?} Applications avec logique métier complexe, besoin de testabilité maximale, Domain-Driven Design (DDD)
